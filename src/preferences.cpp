...
extern "C" {
	GtkWidget* plugin_configure(GtkDialog* dialog)
	{
		...
		gtk_toggle_button_set_active(
			GTK_TOGGLE_BUTTON(pref_widgets.start_with_dot), pref->start_completion_with_dot);
		...
		gtk_toggle_button_set_active(
			GTK_TOGGLE_BUTTON(pref_widgets.start_with_arrow), pref->start_completion_with_arrow);
		...
		g_signal_connect(pref_widgets.start_with_scope_res, "toggled",
			G_CALLBACK(on_pref_dialog_value_changed), NULL);
		gtk_box_pack_start (GTK_BOX(start_with_box), start_with_label, FALSE, FALSE, 5);
		gtk_box_pack_start (GTK_BOX(start_with_box), pref_widgets.start_with_dot, FALSE, FALSE, 8);
		gtk_box_pack_start (GTK_BOX(start_with_box), pref_widgets.start_with_arrow, FALSE, FALSE, 8);
		gtk_box_pack_start (GTK_BOX(start_with_box), pref_widgets.start_with_scope_res, FALSE, FALSE, 8);
		...
		gtk_box_pack_start (GTK_BOX(row_text_max_box), row_text_max_label, FALSE, FALSE, 5);

		GtkObject* row_text_max_adj = gtk_adjustment_new(pref->row_text_max, 10, 512, 1, 10, 0);
		pref_widgets.row_text_max_spinbtn = gtk_spin_button_new(GTK_ADJUSTMENT(row_text_max_adj), 1.0, 0);
		g_signal_connect(pref_widgets.row_text_max_spinbtn, "value-changed", G_CALLBACK(on_pref_dialog_value_changed), NULL);
		gtk_box_pack_start (GTK_BOX(row_text_max_box), pref_widgets.row_text_max_spinbtn, FALSE, FALSE, 5);
		...
		gtk_box_pack_start (GTK_BOX(swin_height_max_box), swin_height_max_label, FALSE, FALSE, 5);

		GtkObject* swin_height_max_adj = gtk_adjustment_new(pref->suggestion_window_height_max, 100, 1080, 1, 10, 0);
		pref_widgets.swin_height_max_spinbtn = gtk_spin_button_new(GTK_ADJUSTMENT(swin_height_max_adj), 1.0, 0);
		g_signal_connect(pref_widgets.swin_height_max_spinbtn, "value-changed", G_CALLBACK(on_pref_dialog_value_changed), NULL);
		gtk_box_pack_start (GTK_BOX(swin_height_max_box), pref_widgets.swin_height_max_spinbtn, FALSE, FALSE, 5);

		gtk_box_pack_start (GTK_BOX(vbox), swin_height_max_box, FALSE, FALSE, 1);

		GtkWidget* compiler_options_label = gtk_label_new(_("compiler options (one option per a line)"));
		...
		gtk_scrolled_window_set_shadow_type(
			GTK_SCROLLED_WINDOW(copt_scrolled_window), GTK_SHADOW_ETCHED_OUT);
		gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(copt_scrolled_window), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
		...
		for(size_t i=0; i<pref->compiler_options.size(); i++) {
			gtk_text_buffer_insert(pref_widgets.options_text_buf, &iter, pref->compiler_options[i].c_str(), -1);
			gtk_text_buffer_insert(pref_widgets.options_text_buf, &iter, "\n", -1);
		}

		g_signal_connect(pref_widgets.options_text_buf, "changed", G_CALLBACK(on_pref_dialog_value_changed), NULL);
